"""
Command Line Interface for Spatial Transcriptomics Cell Cell Agent.
"""
import argparse
import csv
import json
...
def _safe_resolve_path(path_str: str, must_exist: bool = False) -> Path:
    ...
    # Ensure the resolved path is within or equal to the current working directory
    try:
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="spatial-transcriptomics-cell-cell-agent", description="Spatial Transcriptomics Cell Cell Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  SPATIAL TRANSCRIPTOMICS CELL CELL AGENT")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Spatial Transcriptomics Cell Cell Agent Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "serve":
        ...
        print(f"Starting Spatial Transcriptomics Cell Cell Agent API server on http://{args.host}:{args.port}")
